...
console.log(shopStore.getItembyId(Number(route.query["item"])))
        ...
        <template #main>
            <div class="h-full w-full grid grid-rows-5 place-items-center">
                <div class="w-44 h-44 flex-none place-content-center rounded-2xl row-span-2 relative">
                    <div class="rounded-full absolute h-12 w-12 right-0 top-0 place-items-center grid text-lg text-white font-semibold customcolor">
                        {{ item.Price/100 }}€
                    </div>
                    <img :alt="item.Description" class="rounded-full h-44 w-44 object-cover customborder border-4"
                        :src="url + item.Image" />
                </div>
