          ...
          <form class="bg-white shadow-md rounded px-8 pt-6 pb-8 mb-4">
            <div class="mb-4">
              ...
              <div class="flex flex-row">
                <img v-if="(updatedSettings && typeof updatedSettings.Logo === 'string') ||
                  !updatedSettings?.Logo
                  " :src="updatedSettings?.Logo && updatedSettings?.Logo !== ''
    ? url + updatedSettings.Logo
    : url + 'img/logo.png'
    " alt="Augustin logo" class="logo mx-auto my-5" width="200" height="auto" />
                <img v-else :src="newLogo" alt="Augustin logo2" class="logo mx-auto my-5" width="200" height="auto" />
                <input id="logo"
                  class="appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline"
                  type="file" accept="image/png" @change="updateLogo" />
              </div>
            </div>
            <label class="block text-gray-700 text-sm font-bold mb-2 pt-3" for="color">{{ $t('color') }}:</label>
            <div class="flex flex-row">
              <input id="color" v-model="updatedSettings.Color"
                class="appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline"
                type="text" required />
            </div>
            <label class="block text-gray-700 text-sm font-bold mb-2 pt-3" for="color">{{ $t('fontColor') }}:</label>
            <div class="flex flex-row">
              <input id="color" v-model="updatedSettings.FontColor"
                class="appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline"
                type="text" required />
            </div>
            <label class="block text-gray-700 text-sm font-bold mb-2 pt-3" for="mainItem">{{ $t('mainProduct') }}:</label>
            <div class="flex flex-row">
              <select id="mainItem" v-model="updatedSettings.MainItem"
                class="appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline"
                required>
            ...
            <label class="block text-gray-700 text-sm font-bold mb-2 pt-3">{{ $t('transactionCost') }}:</label>
            <div class="flex flex-row justify-evenly">
              <span>
                <label for="RefundFees">{{ $t('yes') }}</label>
                <input id="RefundFees" v-model="updatedSettings.OrgaCoversTransactionCosts" type="radio" checked
                  name="RefundFees" value="true" />
              </span>
              <span>
                <label for="RefundFees">{{ $t('no') }}</label>
                <input id="RefundFees" v-model="updatedSettings.OrgaCoversTransactionCosts" type="radio" name="RefundFees"
                  value="false" />
              </span>
            </div>
            <div class="flex place-content-center">
              <button type="submit" class="p-3 rounded-full bg-lime-600 text-white" @click="updateSettings()">
                {{ $t('confirmation') }}
              </button>
            </div>
          </form>
