      ...
      <main>
        <div class="w-full max-w-md mx-auto mt-4" v-if="item">
          <div class="flex place-content-center justify-between">
            ...
            <button class="p-3 rounded-full mr-3 bg-white bg-lime-600"
          ...
          <form @submit.prevent="updateItem" class="bg-white shadow-md rounded px-8 pt-6 pb-8 mb-4">
            <div class="mb-4">
              <label class="block text-gray-700 text-sm font-bold mb-2 pt-3" for="Name">Name:</label>
              <div class="flex flex-row">
                <input
                  class="appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline"
                  v-model="updatedItem.Name" type="text" id="Name" required />
              </div>
              <label class="block text-gray-700 text-sm font-bold mb-2 pt-3" for="description">Beschreibung:</label>
              <div class="flex flex-row">
                <input
                  class="appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline"
                  v-model="updatedItem.Description" type="text" id="description" required />
              </div>

              <label class="block text-gray-700 text-sm font-bold mb-2 pt-3" for="price">Preis:</label>
              <div class="flex flex-row">
                <input
                  class="appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline"
                  v-model="updatedItem.Price" type="number" id="price" required />
              </div>
              <label class="block text-gray-700 text-sm font-bold mb-2 pt-3" for="image">Bild:</label>
              <div class="flex flex-col">
                <img :src="item.Image.startsWith('img')?apiUrl+item.Image:item.Image" alt="item image" class="productImage" width="100%" height="auto" />
                <input
                  class="appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline"
          ...
          <div v-if="showDeleteModal">
            <div id="defaultModal" tabindex="-1" aria-hidden="false" class="fixed top-0 left-0 right-0 z-50 w-full p-4 overflow-x-hidden flex 
              items-center justify-center overflow-y-auto md:inset-0 h-[calc(100%-1rem)] max-h-full">
...
const updateItem = async () => {
  try {
    store.updateItem(updatedItem.value as Item).then(() => {
