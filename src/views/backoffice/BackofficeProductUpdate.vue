      ...
      <div class="main">
        <div v-if="item" class="w-full mx-auto mt-4">
          ...
          <div class="bg-white shadow-md rounded px-8 pt-6 pb-8 mb-4">
            <div class="mb-4">
              <label class="block text-gray-700 text-sm font-bold mb-2 pt-3" for="Name">{{ $t('name') }}:</label>
              <div class="flex flex-row">
                <input id="Name" v-model="updatedItem.Name"
                  class="appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline"
                  type="text" required />
              </div>
              <label class="block text-gray-700 text-sm font-bold mb-2 pt-3" for="description">{{ $t('description')
                }}:</label>
              <div class="flex flex-row">
                <input id="description" v-model="updatedItem.Description"
                  class="appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline"
                  type="text" required />
              </div>
              <label class="block text-gray-700 text-sm font-bold mb-2 pt-3" for="price">{{ $t('price') }}:</label>
              <div class="flex flex-row">
                <input id="price" v-model="updatedItem.Price"
                  class="appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline"
                  type="number" required />
              </div>
              <label class="block text-gray-700 text-sm font-bold mb-2 pt-3" for="image">{{ $t('image') }}:</label>
              <div class="flex flex-col">
                <img v-if="item.Image" :src="previewImage(item.Image)" alt="item image" class="productImage"
                  width="100%" height="auto" />
                <input id="image"
                  class="appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline"
                  accept="image/png, image/jpeg" type="file" @change="updateImage" />
              </div>
              <label class="block text-gray-700 text-sm font-bold mb-2 pt-3" for="image">{{ $t('isLicenseItem')
                }}</label>
              <label class="inline-flex items-center cursor-pointer mt-4">
                <input id="isLicenseItem" v-model="updatedItem.IsLicenseItem" class="sr-only peer" type="checkbox" />
                <div
                  class="relative w-11 h-6 bg-gray-200 peer-focus:outline-none peer-focus:ring-4 peer-focus:ring-blue-300 dark:peer-focus:ring-blue-800 rounded-full peer dark:bg-gray-700 peer-checked:after:translate-x-full rtl:peer-checked:after:-translate-x-full peer-checked:after:border-white after:content-[''] after:absolute after:top-[2px] after:start-[2px] after:bg-white after:border-gray-300 after:border after:rounded-full after:h-5 after:w-5 after:transition-all dark:border-gray-600 peer-checked:bg-blue-600">
                </div>
                <span class="ms-3 text-sm font-medium">{{ updatedItem.IsLicenseItem ? $t('yes') : $t('no') }}</span>
              </label>
              <div v-if="updatedItem.IsLicenseItem">
                <label class="block text-gray-700 text-sm font-bold mb-2 pt-3" for="image">{{ $t('licenseItem')
                  }}:</label>
                <select v-model="updatedItem.LicenseItem"
                ...
                <label class="block text-gray-700 text-sm font-bold mb-2 pt-3" for="pdf">{{ $t('isPDFLicenseItem')
                  }}</label>
                <label v-if="updatedItem.LicenseItem !== undefined"
                  class="inline-flex items-center cursor-pointer mt-4">
                  <input id="isLicenseItem" v-model="updatedItem.IsPDFItem" class="sr-only peer" type="checkbox" />
                  <div
                    class="relative w-11 h-6 bg-gray-200 peer-focus:outline-none peer-focus:ring-4 peer-focus:ring-blue-300 dark:peer-focus:ring-blue-800 rounded-full peer dark:bg-gray-700 peer-checked:after:translate-x-full rtl:peer-checked:after:-translate-x-full peer-checked:after:border-white after:content-[''] after:absolute after:top-[2px] after:start-[2px] after:bg-white after:border-gray-300 after:border after:rounded-full after:h-5 after:w-5 after:transition-all dark:border-gray-600 peer-checked:bg-blue-600">
                  </div>
                  <span class="ms-3 text-sm font-medium">{{ updatedItem.IsPDFItem ? $t('yes') : $t('no')
                    }}</span>
                </label>
                <div v-if="updatedItem.LicenseItem !== undefined && !updatedItem.IsPDFItem">
                  <label class="block text-gray-700 text-sm font-bold mb-2 pt-3" for="image">{{ $t('licenseGroup')
                    }}:</label>
                  <input id="licenseGroup" v-model="updatedItem.LicenseGroup"
                    class="appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline"
                    type="text" />
                </div>
                <label v-if="updatedItem.LicenseItem !== undefined && updatedItem.IsPDFItem"
                  class="block text-gray-700 text-sm font-bold mb-2 pt-3" for="pdf">{{ $t('pdf item') }}:</label>
                <div v-if="updatedItem.LicenseItem !== undefined && updatedItem.IsPDFItem" class="flex flex-col">
                  <input id="pdf"
                    class="appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline"
                    accept=".pdf" type="file" @change="updatePDF" />
                </div>
              </div>
            </div>

            <div class="flex place-content-center justify-between">
              <button type="submit" class="p-3 rounded-full bg-red-600 text-white" @click="showDeleteModalF">
                {{ $t('delete') }}
              </button>
              <button type="submit" class="p-3 rounded-full bg-lime-600 text-white" @click="updateItem">
                {{ $t('confirmation') }}
              </button>
            </div>
          </div>
                ...
                <div class="relative bg-white rounded-lg shadow dark:bg-gray-700">
                  ...
                  <div class="flex items-start justify-between p-4 border-b rounded-t dark:border-gray-600">
                    ...
                    <button type="button"
                      ...
                      data-modal-hide="defaultModal" @click="showDeleteModal = false">
